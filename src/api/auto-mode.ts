...
export async function getAutoModeStatus(projectId: string): Promise<AutoModeStatus> {
  ...
  const response = await fetch(`/api/auto-mode?projectId=${encodeURIComponent(projectId)}`);
  if (!response.ok) {
...
export async function toggleAutoMode(projectId: string, enabled: boolean): Promise<void> {
...
export async function setAutoModeConcurrency(projectId: string, maxConcurrency: number): Promise<void> {
  const response = await fetch("/api/auto-mode", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
