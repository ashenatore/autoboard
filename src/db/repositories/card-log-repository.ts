import { db } from "~/db";
import { cardLogs } from "~/db/schema";
import { eq, and, gt } from "drizzle-orm";
import { asc } from "drizzle-orm";
import type { CardLog } from "~/db/types";
...
export class CardLogRepository {
  ...
  async getLogsAfterSequence(
    ...
    const results = await db
      ...
      .from(cardLogs)
      .where(
        and(eq(cardLogs.cardId, cardId), gt(cardLogs.sequence, afterSequence))
      )
      .orderBy(asc(cardLogs.sequence));
